...
import { STATUS_META, STATUS_ORDER, type PlotStatus } from "./plot-status-data";
...
export function PlotStatusSelect({
  ...
  // 'unavailable' is admin-only; DB trigger enforces it server-side too
  const options = STATUS_ORDER.filter(
    (s) => s !== "unavailable" || isAdmin || s === value
  );
  ...
  return (
    <div className="mb-[22px]">
      ...
      <Select
        items={options.map((s) => ({ value: s, label: STATUS_META[s].option }))}
        value={value}
        onValueChange={(v) => onChange(v as PlotStatus)}
      >
